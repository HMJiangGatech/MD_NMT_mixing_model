...
if fairseq.__version__=='0.6.0' or fairseq.__version__=='0.6.1':
    from fairseq.tokenizer import Tokenizer
    Binarizer=Tokenizer
else:
    from fairseq.tokenizer import tokenize_line as Tokenizer
...
def main(args):
    ...
    def train_path(lang):
        return ["{}{}".format(trainpref, ("." + lang) if lang else "") for _, trainpref in enumerate(args.trainpref.split(","))]
    ...
    def make_all(lang):
        if args.trainpref:
            for k, trainpref in enumerate(args.trainpref.split(",")):
                ...
                make_dataset(trainpref, outprefix, lang, num_workers=args.workers)
        if args.validpref:
